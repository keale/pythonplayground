...
#überprüfe benachbarte Elemente der sortieren Liste nach Größe
...
for tpl in files_w_same_size:
    for file1, file2 in zip(tpl,tpl[1:]):
        ...
        if hashlib.sha256(open(file1[0],"rb").read()).hexdigest() == hashlib.sha256(open(file2[0],"rb").read()).hexdigest():
            ssf += (file2,)
        else:
            ssf += (file1,)
            if len(ssf)>1:
